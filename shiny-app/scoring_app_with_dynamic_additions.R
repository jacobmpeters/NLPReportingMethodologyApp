...
ui <- shiny::fluidPage(
  ...
  shiny::actionButton("add_question", "Add Another Response"),
  shiny::br(),
  shiny::br(),
...
server <- function(input, output, session) {
  ...
  # Placeholder for dynamic survey questions
  survey_data <- reactiveValues(data = df_questionnaire)

  # Add a new question when button is clicked
  observeEvent(input$add_question, {
    new_question_id <- paste0("response_", nrow(survey_data$data) + 1)
    new_question <- data.frame(
      question = "Please enter your response:",
      option = NA,
      input_type = "text",
      input_id = new_question_id,
      dependence = NA,
      dependence_value = NA,
      required = TRUE
    )
    survey_data$data <- rbind(survey_data$data, new_question)
    create_survey(survey_data$data, survey_id = "text_questions")
  })
